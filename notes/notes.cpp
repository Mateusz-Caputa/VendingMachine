 Product cola("Cola", 1.10);
  ...
  if (VendingSlot *slots_a1 = vendingMachine.getVendingSlot("A1"))
  {
    std::cout << "Selected slot: " << slots_a1->getLabel() << " capacity: " << slots_a1->getCapacity() << "\n";
    slots_a1->addProduct(cola);
  }
  ...
  for (const auto &[slotLabel, vendingSlot] : slots)
  {
    ...
    const std::optional<Product> product = vendingSlot.getFrontProduct();
